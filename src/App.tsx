...
import { MULTIPLIERS, PRODUCTION_RATES, ANIMATION } from './constants/gameConstants';
...
function App() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const autoClickPower = autoClickerCount * PRODUCTION_RATES.AUTO_CLICKER * (1 + MULTIPLIERS.LEVEL_SCALING * autoClickerCount);

      // Calculate extra dirt from Friendly Endermen with level scaling
      const endermanPower = friendlyEndermanCount * PRODUCTION_RATES.FRIENDLY_ENDERMAN * (1 + MULTIPLIERS.LEVEL_SCALING * friendlyEndermanCount);

      // Calculate dirt from Pirate Parrots with level scaling
      const parrotPower = pirateParrotCount * PRODUCTION_RATES.PIRATE_PARROT * (1 + MULTIPLIERS.LEVEL_SCALING * pirateParrotCount);

      // Apply Allay multiplier if there are any Allays
      const allayMultiplier = allayCount > 0 ? 1 + (allayCount * MULTIPLIERS.ALLAY) : 1;
  ...
  const handleClick = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    let clickAmount = clickPower * (1 + MULTIPLIERS.LEVEL_SCALING * clickPower);

    // Проверяем, сработала ли удача от Lucky Cat
    const luckChance = luckyCatCount * MULTIPLIERS.LUCKY_CAT_CHANCE;

    // Генерируем случайное число от 0 до 1 и проверяем, меньше ли оно шанса удачи
    const isLucky = Math.random() < luckChance;
    if (isLucky) {
      // Если удача сработала, умножаем мощность клика
      clickAmount *= MULTIPLIERS.LUCKY_CAT_MULTIPLIER;
    }
  ...
  const totalAutoClickPower = autoClickerCount * PRODUCTION_RATES.AUTO_CLICKER * (1 + MULTIPLIERS.LEVEL_SCALING * autoClickerCount);
  const endermanPower = friendlyEndermanCount * PRODUCTION_RATES.FRIENDLY_ENDERMAN * (1 + MULTIPLIERS.LEVEL_SCALING * friendlyEndermanCount);
  const pirateParrotPower = pirateParrotCount * PRODUCTION_RATES.PIRATE_PARROT * (1 + MULTIPLIERS.LEVEL_SCALING * pirateParrotCount);
  const allayMultiplier = allayCount > 0 ? 1 + (allayCount * MULTIPLIERS.ALLAY) : 1;
